...
function PopUp({ subPop, setSubPop }) {
    const { teachers, classes, sections, updateSection, subjects } = store((state) => state);
            ...
            <div className="bg-white dark:bg-zinc-900 text-gray-800 dark:text-white rounded-xl shadow-xl p-6 w-[90%] max-w-md">
                <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                        <p className="font-medium">{subjects[subPop.subject]?.name}</p>
                        <select
                            value={teacher}
                            disabled={!edit}
...
function Section({ sect }) {
    const { classes, teachers, students, sections, subjects } = store((state) => state)
        ...
        <div className={`"w-full"} relative px-6 py-4 bg-gray-50 dark:bg-zinc-900 transition-colors duration-300`}>
            ...
            <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="students" className="mb-6 border rounded-lg overflow-hidden dark:border-zinc-700">
                    <AccordionTrigger className="px-6 py-4 hover:no-underline bg-white dark:bg-zinc-800">
                        <div className="flex items-center gap-4">
                            <p className="text-lg font-medium text-gray-800 dark:text-gray-200">Students</p>
                            <span className="px-3 py-1 rounded-full bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300">{section.students.length}</span>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent>
                        <div className="flex flex-col">
                            ...
                            {section.students.map((id) => (
                                <div key={id} className="w-full flex px-6 py-3 text-sm items-center border-t border-gray-100 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700/50">
                                    ...
                                    <button className="p-2 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 rounded-full hover:bg-blue-50 dark:hover:bg-blue-900/20" onClick={()=>setStudentPop({student : id})}><FontAwesomeIcon icon={faEye} className="hover:scale-150" /></button>
                                </div>
                            ))}
                        </div>
                    </AccordionContent>
                ...
                <AccordionItem value="teachers" className="mb-6 border rounded-lg overflow-hidden dark:border-zinc-700">
                    <AccordionTrigger className="px-6 py-4 hover:no-underline bg-white dark:bg-zinc-800">
                        <div className="flex items-center gap-4">
                            <p className="text-lg font-medium text-gray-800 dark:text-gray-200">Teachers</p>
                            <span className="px-3 py-1 rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300">{section.subjects.length}</span>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent>
                        <div className="flex flex-col">
                            <div className="w-full flex px-6 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-zinc-700 justify-between">
                                <p className="md:w-[25%] w-[35%]">Subject</p>
                                <p className="md:w-[25%] w-[35%]">Teacher</p>
                                <p className="w-[15%] hidden md:block">Phone</p>
                                <p className="w-[20%] hidden lg:block">Qualification</p>
                                <p className="w-[10%] md:w-[5%]"></p>
                            </div>
                            {section.subjects.map((tws, index) => (
                                <div key={index} className="w-full flex px-6 py-3 text-sm items-center border-t border-gray-100 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700/50 justify-between">
                                    <p className="md:w-[25%] w-[35%] font-medium text-gray-800 dark:text-gray-200">{subjects[tws.subject]?.name}</p>
                                    <p className="md:w-[25%] w-[35%] text-gray-600 dark:text-gray-400">{teachers[tws.teacher]?.name}</p>
                                    <p className="w-[15%] hidden md:block text-gray-600 dark:text-gray-400">{teachers[tws.teacher]?.phone}</p>
                                    <p className="w-[20%] hidden lg:block text-gray-600 dark:text-gray-400">{teachers[tws.teacher]?.qualification}</p>
                                    <button className="w-[10%] md:w-[5%]" onClick={() => setSubPop({ teacherId: tws?.teacher || null, subject: tws.subject, sect: sect })}><FontAwesomeIcon icon={faEye} className="text-blue-800 dark:text-blue-300 hover:scale-125"/></button>
                                </div>
                            ))}
                        </div>
                    </AccordionContent>
