...
const AddSubject = ({ setNewSubject }) => {
    ...
    return (
    <div className={`absolute inset-0 z-20 flex items-center justify-center bg-black/40 top-[10vh]`}>
        <div className={`flex flex-col gap-4 bg-white dark:bg-zinc-800 p-6 rounded-lg shadow-xl w-[90%] max-w-md`}>
            ...
            <input type="text" placeholder="Subject name" className="p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={name} onChange={(e) => setName(e.target.value)}/>
            <div className="flex justify-end gap-3">
                <button className="px-4 py-2 rounded-md bg-gray-300 hover:bg-gray-400 dark:bg-zinc-600 dark:hover:bg-zinc-500 text-gray-800 dark:text-white" onClick={() => setNewSubject(false)}>Close</button>
                <button className="px-4 py-2 rounded-md bg-blue-600 hover:bg-blue-700 dark:bg-blue-400 dark:hover:bg-blue-500 text-white dark:text-black font-semibold" onClick={handleSave}>Save</button>
            </div>
        </div>
    </div>
    )
...
const PopUp = ({ popSubject,setPopSubject }) => {
    ...
    <div className={`absolute inset-0 z-20 flex items-center justify-center bg-black/40 top-[10vh]`}>
        <div className={`flex flex-col gap-4 bg-white dark:bg-zinc-800 p-6 rounded-lg shadow-xl w-[90%] max-w-md`}>
            ...
            <input type="text" placeholder="Subject name" className="p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={name} onChange={(e) => setName(e.target.value)}/>
            <div className="flex justify-end gap-3">
                <button className="px-4 py-2 rounded-md bg-gray-300 hover:bg-gray-400 dark:bg-zinc-600 dark:hover:bg-zinc-500 text-gray-800 dark:text-white" onClick={() => setPopSubject(false)}>Close</button>
                <button className="px-4 py-2 rounded-md bg-red-600 hover:bg-red-700 dark:bg-red-400 dark:hover:bg-red-500 text-white dark:text-black font-semibold" onClick={handleDelete}>Delete</button>
                <button className="px-4 py-2 rounded-md bg-blue-600 hover:bg-blue-700 dark:bg-blue-400 dark:hover:bg-blue-500 text-white dark:text-black font-semibold" onClick={handleSave}>Save</button>
            </div>
        </div>
    </div>
...
function Subject({ leftBar }) {
    ...
    <>
        <div className={`flex flex-col ${leftBar ? 'w-[75%]' : 'w-full'} absolute right-0 top-[10vh] min-h-[90vh] px-6 py-4 dark:bg-zinc-900 bg-gray-100 transition-all duration-300`}>
            <button className="self-start mb-4 flex items-center gap-2 px-4 py-2 rounded-md bg-blue-600 hover:bg-blue-700 dark:bg-blue-400 dark:hover:bg-blue-500 text-white dark:text-black font-semibold shadow" onClick={() => setNewSubject(true)}>
