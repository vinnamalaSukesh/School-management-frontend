...
function Home({ leftBar, classTeacherFor }) {
    const { announcements, classes } = store((state) => state)
    return (
        <div className={`absolute top-10 right-0 transition-all duration-300 ${leftBar ? 'w-[80%]' : 'w-full'} min-h-screen bg-gray-50 dark:bg-zinc-900`}>
            <div className="container mx-auto px-4 py-8">
                <div className="flex flex-col items-start bg-white dark:bg-zinc-800 px-8 py-6 rounded-lg shadow-md border border-gray-200 dark:border-zinc-700 w-fit mx-auto mb-3">
                    <p className=" text-gray-500 dark:text-gray-400 tracking-wide mb-3 mx-auto text-xl">CLASS TEACHER</p>
                    <p className="text-base font-semibold text-gray-800 dark:text-white flex gap-3">
